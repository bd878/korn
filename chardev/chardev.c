...
/*
 * Global vars are declared as static, so they are global within the file
 */
...
int init_module(void)
{
  ...
  printk(KERN_INFO "I was assigned with major number %d. To talk to\n", Major);
  printk(KERN_INFO "the driver, create a dev file with\n");
  printk(KERN_INFO "'mknod /dev/%s c %d 0'.\n", DEVICE_NAME, Major);
  printk(KERN_INFO "Try various minor numbers. Try to cat and each to\n");
