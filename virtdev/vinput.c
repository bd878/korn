...
/*
 * Search the name of vinput device in the vinput_devices ll,
 * which added to vinput_register()
 */
 ...
 * which added af vinput_alloc_vdevice().
 */
...
static struct vinput *vinput_alloc_vdevice(void)
{
  ...
  if (vinput->input == NULL) {
    pr_err("vinput: cannot alloate vinput input device\n");
    err = -ENOMEM;
    goto fail_input_dev;
  }
